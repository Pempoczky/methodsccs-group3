...
library(dplyr)
summary(dat)
dat_filtered <- select(dat, -practice)
...
library(lme4)
m1 = glmer(correct ~ Condition + problem_type + Condition:problem_type + (1|subject_nr) + (1 + problem_type |subject_nr) + (1 + Condition |subject_nr) + (1 + problem_type |trial), data=dat_filtered, family = binomial)
